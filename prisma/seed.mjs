...
// Helper: findUnique + create (avoids transactions — works with HTTP adapter)
async function upsertByUnique(model, where, createData) {
...
async function main() {
  ...
  if (!connStr) {
    throw new Error("DATABASE_URL not set after .env load");
  }

  console.log("Connecting to Neon database (HTTP)…");
  ...
  console.log(`✓ Super Admin (${r1})`);

  // Admin
  const adminHash = await bcrypt.hash("Admin@123", 10);
  const r2 = await upsertByUnique(
    prisma.user,
  ...
  for (const rule of pricingData) {
    const r = await upsertByUnique(prisma.pricingRule, { packageType: rule.packageType }, rule);
    console.log(`  ✓ Pricing ${rule.packageType} (${r})`);
  }
  ...
  for (const v of vehicles) {
    const r = await upsertByUnique(prisma.vehicle, { registration: v.registration }, {
      ...v,
      features: ["Leather Seats", "Sunroof", "GPS Navigation", "Wi-Fi"],
    });
    console.log(`  ✓ Vehicle ${v.registration} (${r})`);
  }

  // Settings
  const settings = [
    { key: "company_name", value: "BMW Rental Rajasthan", type: "string", description: "Company display name" },
    { key: "company_phone", value: "+91-9876543210", type: "string", description: "Primary contact number" },
    { key: "company_email", value: "info@bmwrental.in", type: "string", description: "Primary contact email" },
    { key: "gst_number", value: "", type: "string", description: "GST registration number" },
    { key: "gst_rate", value: "0", type: "number", description: "GST rate percentage" },
    { key: "advance_percentage", value: "30", type: "number", description: "Advance payment percentage" },
  ];
  for (const s of settings) {
    const r = await upsertByUnique(prisma.setting, { key: s.key }, s);
    console.log(`  ✓ Setting ${s.key} (${r})`);
  }
